...
public class DriveTrain extends Subsystem {
	...
	// This is the max rpm we observed the drive train operating at. We used the
	// silverlight can thingy in IE to view the can talon status. Basically
	// follow the guide in CAN Talon software guide section 12.4
	private final double MAX_RPM = 3127.0f;

	// Amount of error allowed for PID controllers. I believe this is +/-, so
	// for turning pid, if setpoint is 90, anything within 88-92 would be
	// considered onTarget. Note you must use setInput to specify the range of
	// possible inputs. See wpilib javadoc for PIDController class, onTarget
	// method.
...
//	private final double RAMP_RATE = 24;
	...
	// These are 'aliases' for the drivetrain talons configured to be masters.
...
//		setDefaultCommand(new DriveControl());
	...
	public DriveTrain() {
		...
		// Configure drivetrain talons to be master/slave on each side of the
		// robot.
		...
		// Assign talons configured to be masters to alias vars.
...
//		leftMaster.setVoltageRampRate(RAMP_RATE);
		...
		// TODO: do we need to set input range? wpilib docs say that for
		// onTarget to work input range needs to be set.
		...
		turnController = new PIDController(10, 0, 0, -3, navx, new PIDOutput() {
			public void pidWrite(double output) {
				...
				// minMove is the min velocity we need to set to make the robot
				// move. There is friction between robot wheels and surface,
				// friction in drive train, inertia, etc.. that needs to be
				// overcome to start moving. This value can change based on
				// surface, robot weight, robot center of gravity, etc..
				double minMove = 500.0f;

				// If we are really close to the setpoint the output from pid
				// controller might result in a really small rpm. Small enough
				// that the robot may not actually move to reach setpoint. Good
				// way to see this is to make a small change to setpoint from
				// current heading.
...
//		server.setQuality(50);
	...
	/**
	 * Drive in a straight line using PID controller to keep us heading in set
	 * direction(angle). Perhaps the term 'Joysticks' in method name is not
	 * ideal. Nothing about this method is tied to joysticks. Just pasing in
	 * speed param which can be read from joystick(y-axis) or in auto-mode via a
	 * command.
	 * 
	 * @param speed
	 *            -1 to 1
	...
	public void velocityTankStraightJoysticks(double speed) {
		...
		// TODO: what if speed is 1/-1 and we need to adjust direction? One side
		// will be at max so adding/subtracting to it will not affect speed on
		// that side. Might be ok.. maybe can test it out and observe?
		rearRightTalon.set((speed * MAX_RPM) + straight);
		rearLeftTalon.set((speed * MAX_RPM) - straight);
	}

	/**
	 * Drive a set distance in a straight line. Use two PID controllers. One to
	 * maintain straight line, other to travel distance.
	 */
	public void velocityDistanceStraight() {
		// We already put the get() values to smartdashboard for these pid
		// controllers in updateSmartDashboard method.
		double straight = straightController.get();
		double distance = -distanceController.get();

		// Combine output of both pid controllers to get a velocity for each
		// side that will keep us in a straight line and travel set distance.
	...
	/**
	 * Set drive train talons to operate in velocity closed loop mode. Also set
	 * PID parameters.
	 */
	...
	/**
	 * Set drive train talons to operate in open loop percent vbus mode. This is
	 * the 'default' mode Talons come on in. It is open loop control. The kind
	 * of driving mode we have done in years past when no encoder is used to
	 * determine speed.
	 * 
	 */
	 ...
	 * mode. Inputs are assumed to be read in from joysticks. Or provide inputs
	 * that match what joysticks outputs would provide.
	 * 
	 * @param speed
	 *            -1 to 1
	 * @param rotate
...
//	}
	...
	public void startTurn(double angle) {
		...
		// testing or maybe robot was bumped by another robot. So we could end
		// up making bad turn.
		navx.reset();
		stopStraight();
		stopDistance();
	...
	/**
	 * Start PID controller to drive straight. Don't need to reset navx angle to
	 * zero here. To drive straight we just need to pick whatever angle the
	 * robot is at when we start the PID controller.
	 * 
	 */
	...
	/**
	 * Talon sensors provide readings in 'native units'. Can't say I exactly
	 * understand what that is, but we need to convert that value to inches so
	 * that we can then use it to do stuff, like travel a set distance.
	 * 
	 * @param nativeUnits
	 *            talon reading of encoder
	...
	/**
	 * Get distance traveled as measured by average of left and right encoders.
	 * 
	...
	private class AvgEncoder implements PIDSource {
		...
		// Need to implement this method else the PID controller that uses this
		// class does not work. Since we are measure distance, type is
